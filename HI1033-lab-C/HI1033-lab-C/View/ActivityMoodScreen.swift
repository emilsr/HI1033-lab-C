...
struct ActivityMoodScreen: View {
    ...
    private let months = DateConstants.months
    ...
    private var activityChart: some View {
        Chart {
            ForEach(activityViewModel.groupedActivities(), id: \.key) { activityType, data in
                ForEach(data, id: \.month) { activity in
                    LineMark(
                        x: .value("Month", activityViewModel.monthOrder[activity.month] ?? 0),
    ...
    private var distanceChart: some View {
        Chart {
            ForEach(activityViewModel.groupedDistances(), id: \.key) { month, data in
                ForEach(data) { distance in
                    BarMark(
                        x: .value("Month", activityViewModel.monthOrder[distance.month] ?? 0),
    ...
    private var moodChart: some View {
        Chart {
            ForEach(moodViewModel.moods) { mood in
                LineMark(
                    x: .value("Month", activityViewModel.monthOrder[mood.month] ?? 0),
